...
def run() -> None:
    ...
    if st.button("Render page"):
        ...
        if not target:
            ...
            incoming_dir = (PROJECT_ROOT / "data" / "incoming").resolve()
            p = (incoming_dir / Path(target).name).resolve()
            if not str(p).startswith(str(incoming_dir)):
                st.error("PDF path must be inside data/incoming")
                return
            if not p.exists():
